...
AI_FRIENDS = {
    ...
    "Елена": {
        ...
        "avatar": "🧠",
        "color": "#4ECDC4",
        "system_prompt": "Ты Елена, 45-летний психолог. Ты мудрая, эмпатичная, всегда внимательно слушаешь и даешь дельные советы. Говоришь спокойно, поддерживающе. Всегда отвечай на вопрос пользователя. Не задавай встречных вопросов. Давай короткий живой ответ в своём стиле."
    },
    "Дед Иван": {
        "age": 72,
    ...
    "Саша": {
        ...
        "avatar": "😎",
        "color": "#FFE66D",
        "system_prompt": "Ты Саша, 19-летний студент. Ты дерзкий, говоришь на языке мемов и сленга молодежи, часто отшучиваешься. Используй современные выражения, иногда добавляешь эмодзи. Всегда отвечай на вопрос пользователя. Не задавай встречных вопросов. Давай короткий живой ответ в своём стиле."
    },
    "Карим": {
        "age": 35,
        ...
        "avatar": "💼",
        "color": "#A8DADC",
        "system_prompt": "Ты Карим, 35-летний бизнесмен. Ты очень практичный, все измеряешь цифрами, говоришь по делу, ценишь эффективность. Часто приводишь примеры из бизнеса и финансов. Всегда отвечай на вопрос пользователя. Не задавай встречных вопросов. Давай короткий живой ответ в своём стиле."
    },
    "Лиза": {
        "age": 28,
        ...
        "avatar": "🎨",
        "color": "#E8B4F3",
        "system_prompt": "Ты Лиза, 28-летняя художница. Ты творческая, эмоциональная, всё видишь в цветах и образах. Говоришь поэтично, часто используешь метафоры и художественные сравнения. Всегда отвечай на вопрос пользователя. Не задавай встречных вопросов. Давай короткий живой ответ в своём стиле."
    }
}
        ...
        if st.session_state.selected_friend is None:
                ...
                if not messages:
                    ...
                    for msg in messages:
                        if msg['sender'] == 'user':
                            ...
                            recipient_text = f" → {msg['recipient']}" if msg.get('recipient') and msg['recipient'] != 'Всем' else ""
                            st.markdown(f"""
            ...
            if st.button("📤 Отправить", type="primary", use_container_width=True):
                if user_message.strip():
                    ...
                    recipient_text = f"→ {st.session_state.selected_friend}" if st.session_state.selected_friend is not None else ""
                    
                    st.session_state.chat_history['group'].append({
                        'sender': 'user',
                        'text': user_message.strip(),
                        'recipient': st.session_state.selected_friend if st.session_state.selected_friend is not None else 'Всем',
                        'timestamp': 'now'
                    })
                    
                    # Generate responses based on recipient
                    if st.session_state.selected_friend is None:
                        # All friends respond
                        ...
                        with st.spinner(f"{st.session_state.selected_friend} думает над ответом..."):
                            response = get_ai_response(
                                st.session_state.api_key,
                            ...
                            # Add friend response to history
                            st.session_state.chat_history['group'].append({
                                'sender': st.session_state.selected_friend,
                                'text': response,
                                'timestamp': 'now'
                            })
            ...
            st.markdown(f"### 💬 Чат с {friend_name} {friend['avatar']}")
            ...
            # Message input
            user_message = st.text_input(
                f"Сообщение {friend_name}:",
                placeholder=f"Напишите {friend_name}...",
                key=f"personal_chat_input_{friend_name}"
            )
